...
##Corrección del query para que verifique si en la carrera del periodo actual ha tenido inscripciones en periodos anteriores
$query = "SELECT
Count(td.[SECTION]) AS total_insc
FROM
dbo.TRANSCRIPTDETAIL AS td
WHERE
td.PEOPLE_ID = '$people_id' AND
td.EVENT_ID = '000000' AND td.ACADEMIC_YEAR+td.ACADEMIC_TERM+td.ACADEMIC_SESSION IN(
SELECT ac.ACADEMIC_YEAR+ac.ACADEMIC_TERM+ac.ACADEMIC_SESSION from dbo.ACADEMIC as ac WHERE ac.PEOPLE_ID = '$people_id' and ac.curriculum = (
SELECT ac1.curriculum FROM ACADEMIC as ac1 WHERE ac1.ACADEMIC_YEAR=$year_actual AND ac1.ACADEMIC_TERM='$periodo_actual' AND ac1.ACADEMIC_SESSION='$plantel_actual' AND ac1.PEOPLE_ID = '$people_id'))
;";
$cmd= new mssqlCommand($query,$con);
...
##Reingreso
$fecha_cierreS = "20/07/2018";##Fecha de cierre Alumnos Reinscritos Semestral
$fecha_cierreC = "08/09/2018";##Fecha de cierre Alumnos Reinscritos Cuatrimestral
...
##Nuevo ingreso
$fecha_cierre2S = "20/07/2018";##Fecha de cierre Alumnos Nuevo Ingreso Semestral
$fecha_cierre2C = "22/09/2018";##Fecha de cierre Alumnos Nuevo Ingreso Cuatrimestral
...
/*BECAS A RENOVAR*/
$recibos    = array(1442427,1409643,1418307,1416328,1415842,1418327,1418657,1414500,1423635,1410901,1425496,1418624,1417642,1418307,1421993,1424300, 1425758,1416328,1415842,1418811,1425782,
    ...
    1442494, 1442543, 1442772, 1442604, 1442491, 1441671, 1441668, 1442540, 1441873, 1441767, 1442819, 1442517, 1443741, 1441924, 1444873, 1442722, 1418299, 1441533,1441657,1453543,1484391,1479235,1484864,1477053);
/*BECAS NUEVAS*/
$recibos2    = array(1453626,1454805,1425276,1426184,1426185,1420691,1420380,1419827,1419366,1419788,1419143,1422895,1422422,1419346,1423244,1412630,1416309,1426048,1426783,1426124,
    ...
    1441696, 1440747, 1442695, 1409723, 1441923, 1441676, 1441875, 1442752, 1445126,1442686,1449648,1440804,1441698,1449559,1456872,030160812,1462304,1469138,1468654,1459857,1473438,1472987,1468717,1466281,1450342,1476158,1477662,1467758,1462070,1477807,1476086,1476212,1467565,1478152,1479235,1479354,1480745,1482814,1477306,1461128,1483692,1465549,1466604,1466224,1481136,1476231,1485026,1485030,1477797,1479706,1475317,1460124,1476038, 1478144,1483988,1486159,1486135,1478146,1478604);
//Se revisa en la bd las exepciones de las bases de Datos
$query = "select matricula from becas_exepciones where plantel = 'PUEBLA' and tipobeca = 'renovacion'";
$cmd = new mssqlCommand($query,$siieCon);
$result = $cmd->ExecuteReader(true);
if (count($result) > 0) {
  $recibos = array_merge($recibos,$result);
}
$query = "select matricula from becas_exepciones where plantel = 'PUEBLA' and tipobeca = 'nueva'";
$cmd = new mssqlCommand($query,$siieCon);
$result = $cmd->ExecuteReader(true);
if (count($result) > 0) {
  $recibos2 = array_merge($recibos2,$result);
}
...
if($folio != NULL)
{
    ...
    foreach ($fd as $value)
    {
        ...
        if($codigo == 'OPCESOLBEC' || $codigo == 'OTCESOLBEC' || $codigo == 'PPDIVERSO')
        {
                ...
                if($solicitud == '')
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            $MSG="14:El tiempo de solicitud de beca se ha terminado.";
                        }
                    }
                }
                else
